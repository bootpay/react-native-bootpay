...
//    [self resetupScripts:configuration];
  ...
  WKWebView * wv;
  #if !TARGET_OS_OSX
    wv = [[WKWebView alloc] initWithFrame:self.bounds configuration: configuration];
#else
    wv = [[BPCWKWebView alloc] initWithFrame:self.bounds configuration: configuration];
#endif // !TARGET_OS_OSX
    ...
    if (_pullToRefreshEnabled) {
        [self addPullToRefreshControl: wv];
    }
    wv.scrollView.scrollEnabled = _scrollEnabled;
    wv.scrollView.pagingEnabled = _pagingEnabled;
      //For UIRefreshControl to work correctly, the bounces should always be true
    wv.scrollView.bounces = _pullToRefreshEnabled || _bounces;
    wv.scrollView.showsHorizontalScrollIndicator = _showsHorizontalScrollIndicator;
    wv.scrollView.showsVerticalScrollIndicator = _showsVerticalScrollIndicator;
    wv.scrollView.directionalLockEnabled = _directionalLockEnabled;
#endif // !TARGET_OS_OSX
    wv.allowsLinkPreview = _allowsLinkPreview;
//    [_webView removeObserver:self forKeyPath:@"estimatedProgress"];
//    [wv addObserver:self forKeyPath:@"estimatedProgress" options:NSKeyValueObservingOptionOld | NSKeyValueObservingOptionNew context:nil];
    wv.allowsBackForwardNavigationGestures = _allowsBackForwardNavigationGestures;

    if (_userAgent) {
      wv.customUserAgent = _userAgent;
    }
#if defined(__IPHONE_OS_VERSION_MAX_ALLOWED) && __IPHONE_OS_VERSION_MAX_ALLOWED >= 110000 /* __IPHONE_11_0 */
...
- (WKWebViewConfiguration *)setUpWkWebViewConfig
{
  ...
  if(self.useSharedProcessPool) {
    wkWebViewConfig.processPool = [[BPCWKProccessPoolManager sharedManager] sharedProcessPool];
  }
...
#if !TARGET_OS_OSX
    if (_pullToRefreshEnabled) {
        [self addPullToRefreshControl: _webView];
    }
...
- (void)visitSource: (WKWebView*) wv
{
    ...
    if (request.URL.host) {
        ...
        [wv loadFileURL:request.URL allowingReadAccessToURL:readAccessUrl];
    }
}
...
#if !TARGET_OS_OSX
- (void)addPullToRefreshControl:(WKWebView *)wv
{
